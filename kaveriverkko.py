"""
Tämä ohjelma analysoi ystävyyssuhteiden verkkoa. Se voi tulostaa kaikki
ystävyyssuhteet, lisätä tietoihin uuden ystävyyssuhteen, tulostaa yhden
ihmisen kaverit ja tulostamaan kahden ihmisen yhteiset kaverit, jos niitä on.
"""
...
def read_friendship_network(filename):
    ...
    # luodaan tyhjä sanakirja, jonka alkiot ovat listoja (nämä huomioidaan
    # myöhemmin)
    network = {}
...
def add_friendship(network, name1, name2):
    ...
    # varmistaa ettei ketään voida lisätä tämän omaksi kaverikseen
    if name1 == name2:
        return

    # lisätään <name1>:n kaverilistaan <name2> ja toisin päin. Jos he ovat jo
    # toistensa listoilla, ei tehdä mitään. Jos nimeä ei ole jo valmiiksi
    # sanakirjassa, lisätään se ja tehdään siitä tyhjä lista.
...
def print_command(network, namelist):
    ...
    # tulostaa kaikki kaverisuhteet aakkosjärjestyksessä
...
def friends_command(network, namelist):
    ...
    # jos syötetty nimi on tuntematon, tulostaa virhe ilmoituksen ja lopettaa
    # toimen.
    if name not in network:
        print(f"Error: '{name}' is an unknown name.")
        return

    # tulostaa yhden nimen kaikki kaverit
    for friend in sorted(network[name]):
        print(friend)
...
def common_friends_command(network, namelist):
    ...
    # tarkistaa, että onko nimi nimilistassa ja tulostaa virheilmoituksen
    for name in namelist:
        if name not in network:
            print(f"Error: '{name}' is an unknown name.")
            return
    ...
    # persoonalla ei voi olla yhteisiä kavereita itsensä kanssa ja tulostetaan
    # virheilmoitus.
    if name1 == name2:
        print(f"{name1} has no common friends with him/herself.")

    # luodaan ensin tyhjä lista yhteisille kavereille. Käydään ensin läpi
    # <name1> kaverit läpi ja jokaisen kaverin kohdalla käydään <name2>
    # kaverit läpi. Jos löydetään yhteinen kaveri, lisätään se listalle.
    else:
        common_friends = []
    ...
    # jos saadun listan pituus on lyhempi kuin 0, tulostetaan teksti, koska
    # näillä ei ole yhteisiä kavereita.
